...
- Classic wall-posting exit: 68 studs/s upward, 32 studs/s outward, 65 percent tangent retention, followed by 0.14 seconds at 65 percent gravity.
...
The two GunZ-inspired orders have deliberately different jobs. Classic wall posting is `wall exit -> slash cancel within 0.22 seconds -> return dash into wall -> fresh Jump`; its 68-stud upward and 32-stud outward launch preserves elevation. The first 0.14 seconds use 65 percent gravity, its return dash has a 0.12-second breathing beat, and input is accepted 0.22 seconds early. Slash-first scale is the fastest vertical route: `air dash into wall -> slash -> Jump`, or `wall run/post -> slash -> Jump`; Jump is accepted from 0.075 through 0.38 seconds after Slash, launches at 82 studs/second upward and 30 outward, then uses 60 percent gravity for 0.16 seconds. It grants one return dash after a 0.14-second beat. Early Jump and return-dash inputs are buffered, not discarded. Same-wall reuse requires 0.28 seconds, 0.45 studs of measured separation, 0.25 studs of moved contact, inward return velocity, a fresh spherecast contact, and another buffered Jump.

The 2026-08-11 owner-provided 120 FPS paired recordings are the calibration record: the sampled GunZ sequence exposes roughly 0.6-0.7 seconds between repeated wall contacts, while the pre-change BLIKK sequence compressed comparable contacts to roughly 0.4-0.5 seconds and travelled visibly farther away from the wall. These values deliberately add readable cadence while preserving fast input through buffering.
