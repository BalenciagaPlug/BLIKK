...
- Fresh manual exit delay: 0.08 seconds after the consumed entry Jump timestamp, with a 0.18-second early-exit buffer.
- Slash cancel: 0.16 seconds after recovery begins; the earned return dash remains available for 0.48 seconds, uses the existing air-dash curve, and bypasses only the ordinary approach-dash cooldown.
- Same-wall return: at least 0.10 seconds, 0.65 studs of outward separation, 0.5 studs of contact-point movement, 5 studs/s back into the wall, a new spherecast hit, and a new buffered Jump.

The intended vertical climb loop is `wall exit -> slash cancel -> return dash into wall -> fresh Jump`. Horizontal extension uses the same cancel resource after an angled wall run. The relaxed return gate supports deliberate repeated scaling while separation, re-entry direction, contact movement, and a new Jump still prevent sticky automatic reattachment.
